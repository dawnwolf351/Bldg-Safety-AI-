        ...
        try:
            while True:
                ...
                if mac:
                    ...
                    # 2. 장비 상태(DeviceState) DB에 저장 로직 추가
                    ...
                    new_state = DeviceState(
                        ...
                        disk_usage=info.get('disk_usage', 0.0),
                        temperature=temp,
                        is_overheated=(temp >= 80.0)  # 80도 이상이면 과열 경고 활성화
                    )
                    db.session.add(new_state)

                    # 3. 한 번의 커밋으로 기기 정보 업데이트와 상태 추가를 동시에 반영
                    db.session.commit()
                    print(f" [TCP 서버] 상태 기록 완료: MAC={mac}, 온도={temp}℃")
            ...
            print(f"️ [TCP 서버] 데이터 처리 중 에러 발생: {e}")
        finally:
            # 연결 종료 시 기기 오프라인 처리
            if mac:
                device = JetsonDevice.query.filter_by(mac_address=mac).first()
