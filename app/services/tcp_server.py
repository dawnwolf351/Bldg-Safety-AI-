        ...
        try:
            while True:
                ...
                if mac:
                    ...
                    # 2. 장비 상태(DeviceState) DB에 저장 
                    # 젯슨이 단순 하트비트(MAC만 전송)를 보낼 때 빈 0값 데이터가 쌓이는 것을 방지
                    if 'cpu_usage' in info or 'temperature_cpu' in info:
                        ...
                        print(f"💓 [TCP 서버] 단순 통신 유지용 하트비트 수신 (DB 기록 생략): MAC={mac}")
